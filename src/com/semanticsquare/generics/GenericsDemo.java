...
public class GenericsDemo <T>/*<T extends List & Serializable>*/ {
	...
	public static void main(String[] args) {		
		...
		//stringStore.set(1);  // gives error .. it gives compile time safety
		...
		list.add(new Double(22.0)); // there is no problem on left .. waise hi laal ho raha hai
		//list.add(new String("22.0")); // this will give error
		...
		//GenericsDemo<ArrayList> list1= new GenericsDemo<>(); // arraylist is both serializable and List also
		...
		// Unbounded Wildcard : better than raw type
		/*
		List<String> strList1 = Arrays.asList("a", "b", "c");
		List<String> strList2 = Arrays.asList("b", "c", "d");
		...
		*/
		...
		// NOTE :Use List instead of array : because in below you will see .. array provide runtime safety and List provide copile time safety .. but Compile time safety is better
		/*
		go(new ArrayList<Integer>());
		go(new Integer[1]);
		*/

		// Generic Methods : Video 280
		/*
		genericMethodsDemo();
		*/
	...
	static void boundedWildcards() {
		...
		Collections.addAll(new ArrayList<Object>(), 1, 2); // refer its code
		Collections.copy(numList, doubleList); // refer its code
		System.out.println("numList after copy: " + numList);
		System.out.println("Collections.disjoint: " + Collections.disjoint(intList, doubleList));  // exact match

		// Type argument inference is Integer with wildcard type version of replaceAll!
		GenericsDemo.replaceAll(numList, 11.5, 44); // exact match
		System.out.println("numList: " + numList);
	...
	// Invariance workaround ~ For harmless scenarios where type safety is not a concern
	static <T extends Number> void invarianceWorkaround(List<T> list , T element) {
		//list.add(new Double(23.3)); // cannot do this ..because it is of bounded type

		// below we can typecast to T
		// T element = (T) new Double(23.3); // typically element of type T will be a method parameter
		list.add(element);
	}
...
//
	...
	// Invariance : compile time safety
	static void go(List<Number> list) {}

	// Covariance : runtime safety
	...
	public static void invalidAggregate(List<?> l1, List<?> l2, List<?> l3) {
		...
		//l3.addAll(l2);   // will give error because it is doing compile time safety check
	}
	...
	public static int getCommonElementsCount(List list1, List list2) {  // list declared Raw type
		int count = 0;
		for (Object element : list1) {  // like in c++ we use auto , in java we use Object
			if (list2.contains(element)) {
				count++;
			}
		}
	...
	public static void rawTypeTest() {
		...
		Double price = prices.get(0);  // will give error here : class java.lang.Integer cannot be cast to class java.lang.Double
	}
...
class HalfIntegrator {
	...
	public static void getPrice(int ISBN, List prices) {  // List is declared Raw type
		prices.add(45);
	}
...
class Store<T> implements Container<T> {
	private T a;
	//private static T a; // cannot do this
