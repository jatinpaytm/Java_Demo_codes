...
public class StreamsDemo {
	...
	private static void imperative() {  // imperative means --> need to declare every method
	...
	private static void declarative() { // declarative code--> only need methods
		System.out.println("\n\nDeclarative: \n");
		/*
		// Stream pipeline (a common structure): 
		     (a) set-up stream source (~ tables in SQL world)
		     (b) 0 or more intermediate operations (~ WHERE clause)(like -> filter , grouping ) -- lazy
...
//		Stream<String> stream = documents.stream()
//				.filter(d -> d.contains("stream"))
//				.map(Indexer::stripHtmlTags)
//				.map(Indexer::removeStopwords);
//				//.forEach(System.out::println);  // they are all lazy because first forEach is executed then filer --> map --> map is executed
//
//		print(stream); // can also do this
	...
	private static void print(Stream<String> stream) {
		...
		//stream.forEach(System.out::println);  // we cannot print it one more time .. because it immediately close the stream.
	}
