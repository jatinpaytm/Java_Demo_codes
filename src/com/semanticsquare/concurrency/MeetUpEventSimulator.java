...
public class MeetUpEventSimulator {
	...
	public static class MeetUpEvent {
		...
		public void notAttending(int guestCount) {			
			if (guestCount == 1) {
				...
				// the below logic is taken from incrementAndGet() method
				while(!updated){
		        	int currentCount = count.get();
					int newCount = currentCount - guestCount;
		            updated = count.compareAndSet(currentCount, newCount); // compare and swap strategy used here
		        }	
			}
		}
	...
	public static void main(String[] args) {
		...
		// 3 users with different tasks are made
